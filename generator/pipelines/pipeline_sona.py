...
class SONAPipeline(DiffusionPipeline, StableDiffusionMixin):
    r"""
    ...
    def prepare_latents(self, batch_size, num_channels_latents, height, width, dtype, device, generator, inputs=None):
        ...
        # Input image
            ...
            latents = self.vae.encode(inputs.to(dtype=dtype)).latent_dist.sample().detach()
    ...
    def __call__(
        ...
        num_early_stop_steps: Optional[int]=None,
        ...
        semantic_direction: Optional[Union[bool, List[bool]]] = False,
        gradual_type: Optional[str] = None,
        ...
        if editing_prompt:
            ...
            enabled_editing_prompts = int(len(editing_prompt) / batch_size)
        ...
        if enable_edit_guidance:
            if '' in editing_prompt:
                    repeated_prompts = [x for item in editing_prompt for x in repeat(item, batch_size)]
            else:
                repeated_prompts = editing_prompt
        ...
        # Add S-step random noise
        if num_early_stop_steps is not None:
            timesteps_for_diffuse = timesteps.flip(0)[:num_early_stop_steps]
            timesteps = timesteps_for_diffuse.flip(0)
            latents = self.scheduler.add_noise(latents, torch.randn_like(latents), timesteps_for_diffuse[-1])
        ...
        id_sem_masks, ood_sem_masks, id_nuis_masks, ood_sem_masks_nuis=[], [], [], []
        for i, t in enumerate(self.progress_bar(timesteps)):
            # expand the latents if we are doing classifier free guidance
            latent_model_input = (
                torch.cat([latents] * (1 + enabled_editing_prompts)) if do_classifier_free_guidance else latents
            )
            ...
            if do_classifier_free_guidance:
                ...
                if self.sem_guidance is None:
                    self.sem_guidance = torch.zeros((num_inference_steps + 1, *noise_pred_uncond.shape))
                ...
                if enable_edit_guidance:
                    ...
                    for c, noise_pred_edit_concept in enumerate(noise_pred_edit_concepts):
                        ...
                        if isinstance(semantic_direction, list):
                            semantic_direction_c = semantic_direction[c]
                        else:
                            semantic_direction_c = semantic_direction
                        ...
                        noise_guidance_edit_mask = copy.deepcopy(noise_guidance_edit_tmp)

                        if semantic_direction_c: # semantic
                            ...
                            if gradual_type == 'linear':
                                ...
                                linear_mask = 1 - (torch.abs(noise_guidance_edit_tmp) / tmp[:, :, None, None])
                                linear_mask = torch.clamp(linear_mask, min=0, max=1)
                                noise_guidance_edit_tmp = (binary_mask *noise_guidance_edit_tmp) * (linear_mask)

                            elif gradual_type == 'sigmoid':
                                binary_mask = torch.where(
                                            torch.abs(noise_guidance_edit_tmp) <= tmp[:, :, None, None],
                                            torch.ones_like(noise_guidance_edit_tmp),
                                            torch.zeros_like(noise_guidance_edit_tmp),
                                ...
                                scale_factor = 10
                                sigmoid_mask = 1 - torch.sigmoid(scale_factor * (torch.abs(noise_guidance_edit_tmp) / tmp[:, :, None, None]))
                                noise_guidance_edit_tmp = (binary_mask *noise_guidance_edit_tmp) * (sigmoid_mask)
                        ...
                        if c==0:  # (+) nuisance original
                            mask = torch.where(
                            torch.abs(noise_guidance_edit_mask) <= tmp[:, :, None, None],
                            torch.zeros_like(noise_guidance_edit_mask), # If the condition is met, black
                            torch.ones_like(noise_guidance_edit_mask), # Else, white
                            )
                            id_nuis_masks.append(mask)
                            nuis_mask=copy.deepcopy(mask)


                        elif c == 1: # (-) semantic original
                            mask = torch.where(
                            torch.abs(noise_guidance_edit_mask) >= tmp[:, :, None, None],
                            torch.zeros_like(noise_guidance_edit_mask), # If the condition is met, black
                            torch.ones_like(noise_guidance_edit_mask), # Else, white
                            )
                            id_sem_masks.append(mask)

                        elif c==2: # (+) semantic ood
                            mask = torch.where(
                            torch.abs(noise_guidance_edit_mask) >= tmp[:, :, None, None],
                            torch.zeros_like(noise_guidance_edit_mask), # If the condition is met, black
                            torch.ones_like(noise_guidance_edit_mask), # Else, white
                            )
                            ood_sem_masks.append(mask)
                            ood_tmp=copy.deepcopy(mask)

                            # nuis_mask=mask*quantile_positions
                            ood_tmp[(nuis_mask == 0) & (ood_tmp == 0)] = 1
                            ood_sem_masks_nuis.append(ood_tmp)

                            noise_guidance_edit_tmp[(nuis_mask == 0) & (noise_guidance_edit_tmp != 0)] = 0
